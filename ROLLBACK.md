...
留**当前的 + 最近 2–3 个**，其余删掉。理由：

- `sw.js` 对页面是**网络优先**，用户拿到的 `index.html` 永远指向当前切片；
- `sw.js` 的 `activate` 又会把 `KEEP` 之外的 `.dat` 从缓存里全删。

所以服务端的老切片实际够不着，只需要覆盖 GitHub Pages 那 ~10 分钟的 HTML 缓存窗口。

2026-09 之前没人管，一路积到 **19 个、6.5MB** 全是死文件。现在
`npm run verify` 第一项设了硬上限（6 个），超了直接报错并打印该跑的 `git rm`。
