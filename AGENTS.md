...
- **不做 UGC**：不接受用户上传的文本/图片/头像。头像只能从预设 emoji + 预设背景色里选。
  ...
  - **唯一例外：学习笔记**（2026-09 站长拍板）。用户能在词句卡片上写给自己看的笔记，
    成立的前提是两条硬承诺，破了任何一条这个例外就不成立：
    1. **永不公开展示**——只存本地 + 随进度同步到自己的账号，不出现在任何他人可见之处，
       因此零审核负担。笔记内容**只许**出现在 `/api/progress` 的请求体里。
    2. **用户文本永不变成 DOM**——一律 `textContent`，一个字符都不拼进 `innerHTML`
       （`_esc()` 不转义单引号，见 §6，别给它机会）。
    上限：单条 200 字、共 500 条。笔记跟着 `_progressDoc()` 整包 PUT 到 D1，不封顶的话
    一个用户就能把同步文档撑到几 MB，之后每次同步都要整包来回传。
    `tools/verify-notes.mjs` 逐条钉死以上四点。
  - **不要**拿这个例外当先例去加评论区、留言板、公开笔记——那是另一回事：
    要审核后台、要举报、要限流、要注销时清理，运维负担完全不同。
