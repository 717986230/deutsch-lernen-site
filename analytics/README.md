...
> **密钥猜错会被限流**：每 IP 1 小时最多猜错 20 次，超了回 429。
> 这个端点是一把「猜对就全看到」的共享密钥，之前**一次都不限流**，可以拿字典无限试；
> 返回的虽然只是聚合数据（不含原始 IP），站点流量画像也不该谁都能翻。
> 密钥比对改成逐字符全比完再判（跨公网的时间差其实被网络抖动淹没，真正起作用的是限流，
> 这几行只是把问题彻底关掉）。没配 `STATS_KEY` 时一律 403，不会「不传 key 就放行」。
